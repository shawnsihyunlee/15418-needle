...
_DEVICES = [
    ...
    pytest.param(ndl.msl(), marks=pytest.mark.skipif(not ndl.msl().enabled(), reason="No MSL")),
    pytest.param(ndl.mps(), marks=pytest.mark.skipif(not ndl.mps().enabled(), reason="No MPS"))
]
_DEVICE_IDS = ["cpu", "cuda", "msl", "mps"]
...
def submit_new_nd_backend():
    ...
    if ndl.msl().enabled():
        devices.append(ndl.msl())
    if ndl.mps().enabled():
        devices.append(ndl.mps())
    if len(devices) == 1:
        print('No GPU/MSL available; running CPU-only submissions.')
