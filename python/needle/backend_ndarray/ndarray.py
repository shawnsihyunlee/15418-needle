...
def cuda_device_count() -> int:
    ...
    try:
        from . import ndarray_backend_cuda  # type: ignore[attr-defined]
        return ndarray_backend_cuda.get_device_count()
    except (ImportError, AttributeError, Exception):
        return 0
    

def msl() -> BackendDevice:
    """Return msl device"""
    try:
        from . import ndarray_backend_msl  # type: ignore[attr-defined]

        return BackendDevice("msl", ndarray_backend_msl)
    except ImportError:
        return BackendDevice("msl", None)
    
    
def mps() -> BackendDevice:
    """Return mps device"""
    try:
        from . import ndarray_backend_mps  # type: ignore[attr-defined]
        return BackendDevice("mps", ndarray_backend_mps)
    except ImportError:
        return BackendDevice("mps", None)
...
def all_devices() -> list[BackendDevice]:
    ...
    devices = [cpu(), cuda(), cpu_numpy(), msl(), mps()]
    # Add all available CUDA devices
    num_cuda = cuda_device_count()
    for i in range(num_cuda):
        devices.append(cuda(i))
...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        # Let NDArray.make compute the compact strides for the new shape
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        """
        non_singleton_dims = [i for i, s in enumerate(self.shape) if s != 1]
        for i in non_singleton_dims:
            if new_shape[i] != self.shape[i]:
                raise ValueError("Broadcast size mismatch")
        
        new_strides = tuple(
            self.strides[i] if self.shape[i] != 1 else 0 for i in range(len(new_shape))
        )
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        new_shape = tuple(
            math.ceil((slices[i].stop - slices[i].start) / slices[i].step)
            for i in range(len(slices))
        )
        new_strides = tuple(
            self.strides[i] * slices[i].step for i in range(len(slices))
        )
        new_offset = self._offset + reduce(
            operator.add, 
            (self.strides[i] * slices[i].start for i in range(len(slices))), 
            0
        )
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        # The offset is the sum over every flipped axis of 
        # (shape[axis]-1)*stride[axis]
        new_offset = self._offset + reduce(
            operator.add, 
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        """
        for axis, (pad_left, pad_right) in enumerate(axes):
            new_shape = list(self.shape)
            new_shape[axis] += pad_left + pad_right
            new_array = full(
                tuple(new_shape), 
                fill_value=0, 
                device=self.device
            ...
            slices = tuple(
                slice(pad_left, pad_left + self.shape[i]) if i == axis else slice(0, self.shape[i])
