...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        adjoint = 0 if not partial_adjoints else sum_node_list(partial_adjoints)
        node.grad = adjoint

        for i, inp in enumerate(node.inputs):
            if inp.requires_grad:
                inp_contribution = node.op.gradient_as_tuple(adjoint, node)[i]
                if inp in node_to_output_grads_list:
                    node_to_output_grads_list[inp].append(inp_contribution)
                else:
                    node_to_output_grads_list[inp] = [inp_contribution]
